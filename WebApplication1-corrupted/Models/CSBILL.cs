...
namespace WebApplication1
{
    ...
    public partial class CSBILL
    {
        ...
        public string TAXCODE { get {
                string retval = _taxrate.Where(x => x.EFFECTIVE_START <= this.ENTDATE && x.EFFECTIVE_END >= this.ENTDATE && x.TAXRATE == this.TAXRATE).Select(y => y.TAXTYPE).FirstOrDefault();
        ...
        public string TAXRCODE { get {
                string retval = _taxrate.Where(x => x.EFFECTIVE_START <= this.ENTDATE && x.EFFECTIVE_END >= this.ENTDATE && x.TAXRATE == this.TAXRATE).Select(y => y.TAXRCODE).FirstOrDefault();
